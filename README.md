...
No API key required: set `SIFT_LLM_PROVIDER=local` and answers are generated by
a Hugging Face model on your machine. A key buys you a better generator and the
two evaluation metrics that genuinely need an LLM judge.
...
The answer-level metrics are measured too, using a local generator rather than a
key (`benchmarks/latest_answers.json`, Qwen2.5-3B-Instruct, greedy):

| Metric | Score | |
|---|---|---|
| `citation_validity` | **1.0** | 18 citations emitted, 0 invalid |
| `abstention_rate` | **1.0** | all 4 unanswerable questions declined |
| `false_abstention_rate` | 0.0625 | 2 of 32 — see below |

Citation validity of 1.0 is a property of the *design*, not of the model: answers
are generated against numbered passages and every marker is validated against the
retrieved set afterwards, so an invented citation is stripped before it can reach
a user. A 3B model emitting zero invalid citations over 36 questions is evidence
that closed-vocabulary citation works.

The two false abstentions are worth separating. On `q017` retrieval never found
the gold document, so declining was the **correct** behaviour — the metric counts
it as a false abstention because it only knows the question was answerable in
principle. `q009` is the real one: the gold document was retrieved and the model
declined anyway. That is a limitation of a 3B generator, and it is why these
numbers are labelled with the model that produced them.

Only **faithfulness** and **answer_relevancy** still need a judge — one is
entailment checking over each claim, the other generates questions from the
answer. There the model's output *is* the measurement, so a small model grading
its own work is worth nothing. Those two remain unmeasured and are reported as
unmeasured.
...
**You do not need an API key to run any of this.** `SIFT_LLM_PROVIDER=local`
runs a Hugging Face instruct model on your own machine, so `POST /query` returns
a cited answer on a fresh clone instead of an error telling you to go and buy
something:

```bash
SIFT_LLM_PROVIDER=local uvicorn sift.api:app     # or python -m eval.retrieval_eval
```

That also decides which metrics need money. Citation validity and abstention are
properties of a *generated answer*, but neither asks a model for an opinion —
citation validity is computed by our own code against the retrieved set, and
abstention is a property of the text. They need an answer, not a judge, so they
are measured locally and for free. Only `faithfulness` and `answer_relevancy`
genuinely need a judge, because there the model's output *is* the measurement,
and a small model grading its own work is worth nothing.

Numbers produced this way are labelled with the model that produced them.
Citation discipline is a property of the generator, and quoting it bare would
imply the pipeline guarantees something the generator happens to be doing.
...
- **Two of the four RAGAS metrics are still unmeasured.** `faithfulness` and
  ...
  above.
- **Answer quality is measured under a 3B local model.** Citation validity and
  abstention are real measurements, but of this pipeline *with this generator*.
  A larger model would very likely fix the one genuine false abstention; the
  citation-validity result should transfer, since it is enforced by code rather
